...
This can only be applied to geometries that have the following characteristics, they have a `stat` other than `"identity"`, and have some sense of direction dictated by the `orientation` parameter. `geom_*sideboxplot` and `geom_*sideviolin` are *not* included in this update because the `orientation` largly depends the aesthetic mapping of the layer, and these two geometries could go either way depending on position scales of `x` and `y`.
