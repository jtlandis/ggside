...
#' @name StatSummarise
#' @aliases StatSummarisex StatSummarisey StatSummarizex StatSummarizey
StatSummarise <- ggplot2::ggproto("Summarise",
                                Stat,
                                required_aes = c("x|y",  "domain|domain"),
                                by = NULL,
                                compute_panel = function(self, data, scales,
                                  ...
                                  fun.data <- data %>%
                                    group_by_(.dots = by) %>%
                                    dplyr::summarise(summarise = call_f(fun, domain))
                                  data <- left_join(data, fun.data, by = by)
...
#' @rdname stat_summarise
StatSummarisey <- ggplot2::ggproto("Summarisey",
                                   StatSummarise,
                                   by = "y")
#' @rdname stat_summarise
StatSummarisex <- ggplot2::ggproto("Summarisex",
                                   StatSummarise,
                                   by = "x")
#' @rdname stat_summarise
StatSummarize <- ggplot2::ggproto("Summarize",
                                  StatSummarise)
#' @rdname stat_summarise
StatSummarizey <- ggplot2::ggproto("Summarizey",
                                   StatSummarize,
                                   by = "y")
#' @rdname stat_summarise
StatSummarizex <- ggplot2::ggproto("Summarizex",
                                   StatSummarize,
                                   by = "x")
