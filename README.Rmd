...
* `geom_xsidetext()` is using the incorrect `ggproto` Geometry in its layer. ![#19](https://img.shields.io/badge/fixed-master-green.svg?style=flat-square)
* `ggplot2::facet_wrap()` and `ggplot2::facet_grid()` will cause messages to be print to the screen (#20). While this doesn't break code, it is annoying and was a bug that went undetected until switching to the latest version of `testthat`. ![#20](https://img.shields.io/badge/fixed-master-green.svg?style=flat-square) #20
