...
The following section will summarize issues that are present on the current CRAN release. These will either be fixed on the main branch of this git repository, or currently in development to be fixed on one of the development branches. The current CRAN version of `ggside` is v0.2.2.


* [\#40
     ![](https://img.shields.io/badge/fixed-dev-green.svg?style=flat-square)](https://github.com/jtlandis/ggside/issues/40) Side scales cannot be reliably transformed. This is "fixed" under the dev version of `ggside`, but the overall new approach in the development branch is not in a place where I am comfortable to release just yet. This will be added in `ggside 0.3.0`.
